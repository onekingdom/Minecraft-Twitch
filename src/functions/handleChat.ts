import { CommandDatabase } from "../classes/command-database";
...
export async function HandleChatMessage(chatMessage: ChatMessageEvent) {
  ...
  if(!foundCommand) console.log("Command not found")
  ...
  if (foundCommand) {
    ...
    if(foundCommand.action) {
      const x = await handle_action(foundCommand.action, args, +broadcaster_user_id, chatter_user_name);
