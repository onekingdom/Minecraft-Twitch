...
export default async function handle_banned_song_add({broadcaster_id, broadcaster_name, chatter_id, chatter_name, song, user_id}: BannedSong) {
  ...
  const is_banned = await spotifyDB.check_song_banned(user_id, id);
  ...
  const spotify_settings = await spotifyDB.get_spotify_settings(user_id);
